...
| 1 | Manifest: parse + validate (TDD) | done | `src/iris/skills/manifest.py` (`split_frontmatter`, `parse_skill_md`, `parse_sidecar`, `validate_skill`); `Skill` widened (all fields defaulted); `tests/test_skill_manifest.py` (24) written failing first |
| 2 | Discovery + registry (TDD) | done | `src/iris/skills/registry.py` (`SkillRegistry`, `RegistryConflict`, `open_registry`); four sources in precedence order; conflicts reported; no caching; `engine.py` wired; `tests/test_skill_registry.py` (20) |
| 3 | Policy enforcement (TDD) | done | `src/iris/skills/policy.py` (`SkillPolicy`, `policy_for`); `IrisState.active_skills`; `dispatch()` + `tool_schemas()` enforce/filter; denials in the turn trace; `tests/test_skill_policy.py` (15) |
| 4 | Script boundary (TDD) | done | `src/iris/skills/runner.py` (AST `pre_screen`, `resolve_script`, `run_script`) + `guard.py` (`screen_script` via `JevClient`) + the `skill_run` tool; `tests/test_skill_runner.py` (31) and `tests/test_skill_tool.py` (13) |
| 5 | CLI (TDD) | done | `src/iris/cli/skills.py` + `skills` command in `cli/main.py`; command registry now exactly `{chat, doctor, skills, version}` (assertion updated); `tests/test_skills_cli.py` (11) |
...
5. **`asyncio.create_subprocess_exec` → a worker thread around `subprocess.run`.**
   ...
   Windows `Selector` event loop that `iris.api` selects for psycopg, so a skill
...
- **Closed after P4 (owner-selected cleanup, 2026-09-24):** the dead recall-scoring
  knobs were deleted from `src/iris/config.py`. `hybrid_top_k: int = 20` was the
