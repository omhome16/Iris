...
1. **Prompt version + model version on every trace span.** The vault: "Can't
   tell which prompt caused it" is a top observability failure, and "every trace
   carries prompt version, model version, adapter version and tenant". Iris's
   trace carries `stages_ms`, judgment events and an args hash, but not a prompt
   version — so a quality change cannot be attributed to a prompt edit.
2. **A model-free retrieval gate in CI.** Two-speed gating: deterministic
...
3. **`recall@pool` logged continuously**, not only in eval — the vault calls this
   the single most valuable debugging affordance, because it exposes the retrieval
   ceiling before anyone blames the generator.
4. **A memory-OFF ablation baseline.** The vault: "memory was never evaluated
   with an OFF baseline so nobody noticed it hurt." Iris has an ablation study
   but it predates the JEV-era capture node.
5. **Deferred tool loading / tool search** once the surface passes ~20 tools.
   Iris declares the visible surface and reports it, but does not yet let the
   model request a subset by namespace.
6. **Hash-pinning for third-party skill packages with re-approval on change.**
...
7. **A kill switch and a best-so-far answer path** on a halted turn — the vault
   wants every agent to "degrade instead of failing".
8. **TTFT/TPOT split** rather than total latency, so p99 can be attributed.
