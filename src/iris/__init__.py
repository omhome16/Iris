...
`harness()` and `Harness`. The boot wiring lives in `iris.engine` (that name,
not `iris.harness`, because a submodule and a package attribute cannot share
one) and pulls in LangGraph, asyncpg and the rest — so it is re-exported lazily,
keeping `import iris` (and therefore `iris.cli`) cheap.
"""
...
def __getattr__(name: str):
    """Lazy re-export. Uses `importlib` deliberately: `from iris import harness`
    ...
    if name in {"harness", "Harness"}:
        ...
        module = importlib.import_module("iris.engine")
