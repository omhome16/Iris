"""The engine's boot path — public as `iris.harness()`.

Why this module is `iris.engine` and not `iris.harness`: a submodule and a
...
`iris.harness()` promises, so the implementation lives here and the public name
is re-exported by `iris/__init__.py`.

Before P2 this wiring lived inside `iris.api`'s FastAPI lifespan, which meant
...
from iris.scheduler import build_scheduler, owner_sleep_hour, reschedule_nightly
from iris.skills.registry import open_registry
from iris.tasks import TaskScheduler, TaskStore
from iris.toolpolicy import parse_overrides
from iris.trace import TraceLogger

log = logging.getLogger("iris.harness")
...
async def harness(
    *,
    ...
    async with AsyncExitStack() as stack:
        ...
        from iris.agents.orchestrator import Orchestrator
        ...
        if settings.computer_enabled:
            from iris.computer import computer_from_settings
